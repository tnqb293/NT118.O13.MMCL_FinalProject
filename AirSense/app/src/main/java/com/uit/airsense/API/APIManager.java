...
public class APIManager {
            ...
            public void onResponse(Call<JsonArray> call, Response<JsonArray> response) {
                if(response.isSuccessful())
                {
                    ...
                    if(data.size() == 0)
                    {
                        ...
                        empty.addProperty("x", 0);
                        empty.addProperty("y", 0);
                    }
    ...
    public static void getWindSpeedChart(JsonObject request, DataChartCallback callback)
    {
        Call<JsonArray> call = apiData.getChartWindSpeedData(request);
        call.enqueue(new Callback<JsonArray>() {
            @Override
            public void onResponse(Call<JsonArray> call, Response<JsonArray> response) {
                if(response.isSuccessful())
